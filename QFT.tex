...
These mass singularities arises from collinear emissions of quarks and/or gluons in QCD. The correction to the order of $\alpha_s$ diverge which in turn give rise to mass singularities. However, in QED these collinear photon emission cancel.
...
\begin{equation}
\phi \rightarrow e^{i\alpha ^a \tau ^a} e^{\frac{i}{2}\beta} \phi
\end{equation}
...
\subsection{Questions}
CP-violation? Is it possible to obtain a situation when the 6.2 premises is not valid?
...
\subsection{Questions}
$\xi=0 \rightarrow$ Goldstone boson has the couplings it has in the ungauged model? Question 7.2: Will it as well lead to unphysical polarizations?
