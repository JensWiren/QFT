...
\begin{equation}
\average{A_{\mu}^{a}(x)A_{\nu}(b)} = \int \dfrac{d^4k}{(2\pi)^ 4} \parentheses{ \dfrac{-ig_{\mu\nu}}{k^2}} \delta^{ab}e^{-ik\cdot (x-y)}
\end{equation}
...
There is a flaw in the previous argument: we had to assume that a gauge boson was transverse and while this is true in QED this it not necessarily true in a Yang-Mills theory. To remedy this Faddeev and Popov introduced ghost fields and ghost terms in the Lagrangian. These ghost field have the wrong relation between spin and statistics and thus only show up as virtual particles and never as physical observable particles, however they also have the consequence of cancelling the unwanted unphysical polarization states.
...
The ghost contribution exactly cancels of the contributions of non-physical gauge bosons. In general the ghost particles serve as negative degrees of freedom to cancel the effects of non-physical polarization states of the gauge bosons. (Vi ar inte helt nojda med detta svar, fyll garna i).

\subsection{What diagrams contribute to the gauge boson self-energy}

REF: page 522, figure 16.7

\subsection{Why is the appearence of gauge fixing parameter $\xi$ not a problem for gauge boson selg-energy? }

The fact that the boson self-energy depends on the gauge does not contradict the general theorem that the S-matrix elements are independent of $\xi$. The reason for this is that the full set of one-loop corrections to a gauge theory S-matrix element always involves a number of different \textit{radiative corrections} to vertices and propagators. (at the bottom of page 526) 
